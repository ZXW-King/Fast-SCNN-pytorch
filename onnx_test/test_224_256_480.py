...
def GetArgs():
    ...
    parser.add_argument("--model", type=str, default="onnx_model/fast_scnn_wire_best_s7.onnx",help="")
    ...
    if img_path.endswith(".txt"):
        with open(img_path) as f:
            ...
            for img in tqdm(files,desc='Processing'):
                ...
                if "s4" in args.model:
                    save_img_path = os.path.join(root_path,"s4",img_name)
                elif "s5" in args.model:
                    save_img_path = os.path.join(root_path,"s5",img_name)
                elif "s6" in args.model:
                    save_img_path = os.path.join(root_path,"s6",img_name)
                elif "s7" in args.model:
                    save_img_path = os.path.join(root_path,"s7",img_name)
                else:
                    raise "txt name error!"
